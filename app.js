...
function renderRoleCard(data) {
  ...
  app.innerHTML = `<main class="role-page"><section class="role-card"><div class="tag">RUNDA ${data.round} · ${data.name.toUpperCase()}</div><div class="card-seal" id="role-icon">✦</div><div class="eyebrow">Ditt förseglade rollkort</div><h1 id="role-name">Är du redo?</h1><p class="intro">Se till att du är ensam innan du bryter sigillet. Inga tjuvkikare.</p><button class="button pink reveal" id="reveal">Bryt sigillet</button><div class="secret" id="secret"><img class="private-role-art" src="${role.image}" alt="${role.name}"><div class="role-secret-copy"><span class="role-alignment">${role.icon} ${role.alignment}</span><h2>${role.name}</h2><p>${role.text}</p></div></div><p class="tiny">Din länk är densamma i nästa runda. Ladda om sidan när arrangören har startat omgången.</p></section></main>`;
  ...
  document.querySelector('#reveal').onclick = event => { const showing = secret.classList.toggle('show'); name.textContent = showing ? role.name : 'Är du redo?'; icon.textContent = showing ? role.icon : '✦'; event.currentTarget.textContent = showing ? 'Försegla igen' : 'Bryt sigillet'; };
}
